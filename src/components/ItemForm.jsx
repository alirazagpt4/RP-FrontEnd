...
export default function ItemForm({ onSaved, socket, currentUser }) {
  ...
  const [isWhManual, setIsWhManual] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchActiveColors = async () => {
      try {
        const res = await axios.get('/colors/active');
        setActiveColors(Array.isArray(res.data) ? res.data : res.data.data || []);
    ...
    const fetchActiveSizes = async () => {
      try {
        const res = await axios.get('/sizes/active');
        setActiveSizes(Array.isArray(res.data) ? res.data : res.data.data || []);
    ...
    // Click outside handler to close custom dropdowns safely
    const handleClickOutside = (event) => {
      if (colorRef.current && !colorRef.current.contains(event.target)) setShowColorDropdown(false);
      if (sizeRef.current && !sizeRef.current.contains(event.target)) setShowSizeDropdown(false);
    };
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  // --- LOGIC: Handle Type Change & Claim Amount Reset ---
  const handleTypeChange = (selectedType) => {
    setType(selectedType);
    if (selectedType === 'claim') {
      setClaimAmount(0); // Safely forces 0 when Claim is chosen
    } else {
      setClaimAmount(500); // Default placeholder for Repairs
    }
  };

  // --- LOGIC: Auto Warehouse Selection ---
  useEffect(() => {
    if (isWhManual) return;
  ...
  const validate = () => {
    ...
    // Street Address dependency removed completely from here
  ...
  const resetForm = () => {
    ...
    setColorSearch(''); setSizeSearch('');
    setCustomerName(''); setBillingAddress(''); setCustomerMobile('');
    setProvince(''); setCity('');
    setProblemDescription(''); setClaimAmount(0); setType('');
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('streetAddress', ''); // Always posts empty to backend seamlessly
  ...
  // Filter logic for live lists
  const filteredColors = activeColors.filter(c => c.name.toLowerCase().includes(colorSearch.toLowerCase()));
  const filteredSizes = activeSizes.filter(s => s.name.toLowerCase().includes(sizeSearch.toLowerCase()));
      ...
      <div className="w-full max-w-full h-auto bg-white shadow-xl rounded-2xl border border-slate-200 overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-8 grid grid-cols-12 gap-8">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                {['repair', 'claim', 'local repair'].map((t) => (
                  <button key={t} type="button" onClick={() => handleTypeChange(t)} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all border-2 ${type === t ? 'bg-sky-600 text-white border-sky-600 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:border-sky-300'}`}>{t}</button>
                ))}
              </div>
            ...
            {type !== 'local repair' && (
              <div className="grid grid-cols-1 gap-4 bg-sky-50/50 p-4 rounded-2xl border border-sky-100">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    {['WHFSD', 'WHLHR'].map((wh) => (
                      <button key={wh} type="button" onClick={() => {setWarehouse(wh); setIsWhManual(true);}} className={`flex-1 py-2 rounded-lg text-[10px] font-black border transition-all ${warehouse === wh ? 'bg-sky-700 text-white border-sky-700' : 'bg-white text-slate-400 border-sky-200 hover:border-sky-400 cursor-pointer'}`}>{wh}</button>
                    ))}
                  </div>
                </div>
                {type !== 'claim' && (
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-1">
                ...
                <input value={articleNo} maxLength="6" onChange={(e) => setArticleNo(e.target.value)} className={`w-full border-b-2 outline-none py-2 font-bold text-sm ${errors.articleNo ? 'border-rose-500' : 'border-slate-200'}`} placeholder="e.g. 012098" />
              ...
              <div className="col-span-1 relative" ref={colorRef}>
                <label className="text-[10px] font-black text-slate-400 uppercase">Color *</label>
                <div onClick={() => setShowColorDropdown(true)} className={`w-full border-b-2 py-2 font-bold text-sm cursor-pointer min-h-[38px] flex items-center justify-between ${errors.color ? 'border-rose-500' : 'border-slate-200'}`}>
                  <span>{color || <span className="text-slate-300 font-normal">Select Color...</span>}</span>
                </div>
                {showColorDropdown && (
                  <div className="absolute z-30 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl p-2 max-h-52 overflow-hidden flex flex-col">
                    <input type="text" placeholder="Search Color..." value={colorSearch} onChange={(e) => setColorSearch(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs font-bold outline-none mb-2" />
                    <div className="overflow-y-auto flex-1 space-y-0.5">
                      {filteredColors.length > 0 ? filteredColors.map((c) => (
                        <div key={c.id} onClick={() => { setColor(c.name); setShowColorDropdown(false); setColorSearch(''); }} className="p-2 text-xs font-bold text-slate-700 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors">
                          {c.name}
                        </div>
                      )) : <div className="text-[10px] p-2 text-slate-400 font-semibold text-center">No color found</div>}
                    </div>
                  </div>
                )}
              ...
              <div className="col-span-1 relative" ref={sizeRef}>
                <label className="text-[10px] font-black text-slate-400 uppercase">Size *</label>
                <div onClick={() => setShowSizeDropdown(true)} className={`w-full border-b-2 py-2 font-bold text-sm cursor-pointer min-h-[38px] flex items-center justify-between ${errors.size ? 'border-rose-500' : 'border-slate-200'}`}>
                  <span>{size || <span className="text-slate-300 font-normal">Select Size...</span>}</span>
                </div>
                {showSizeDropdown && (
                  <div className="absolute z-30 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl p-2 max-h-52 overflow-hidden flex flex-col">
                    <input type="text" placeholder="Search Size..." value={sizeSearch} onChange={(e) => setSizeSearch(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs font-bold outline-none mb-2" />
                    <div className="overflow-y-auto flex-1 space-y-0.5">
                      {filteredSizes.length > 0 ? filteredSizes.map((s) => (
                        <div key={s.id} onClick={() => { setSize(s.name); setShowSizeDropdown(false); setSizeSearch(''); }} className="p-2 text-xs font-bold text-slate-700 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors">
                          {s.name}
                        </div>
                      )) : <div className="text-[10px] p-2 text-slate-400 font-semibold text-center">No size found</div>}
                    </div>
                  </div>
                )}
          ...
          <div className="col-span-12 lg:col-span-3 space-y-6">
            ...
            {/* CONDITIONAL CLAIM AMOUNT SECTION */}
            {(type === 'repair' || type === 'local repair') && (
              <div className="space-y-2 p-4 bg-amber-50/60 border border-amber-200 rounded-2xl animate-in fade-in duration-300">
                <label className="text-[10px] font-black text-amber-900 uppercase tracking-wider block">Courier Charges *</label>
                <div className="flex gap-2">
                  {[500, 0].map((amt) => (
                    <button key={amt} type="button" onClick={() => setClaimAmount(amt)} className={`flex-1 py-2 rounded-xl text-xs font-black transition-all border ${claimAmount === amt ? 'bg-amber-700 text-white border-amber-700 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-amber-300'}`}>
                      Rs. {amt}
                    </button>
                  ))}
                </div>
